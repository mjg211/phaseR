...
ex1.out <- do.call(nullclines, c(deriv=phaseR::example1, ex1.pars))

# Test example 10
ex10.pars <- list(xlim = c(-5, 5), ylim = c(-5, 5), add = F, points=11)
ex10.out <- do.call(nullclines, c(deriv=phaseR::example10, ex10.pars))

test_that("behavior equal to reference behavior", {
  expect_equal_to_reference(ex1.out["dy"], "test-nullclines_ref-ex1.rds")
  expect_equal_to_reference(ex10.out[c("dx","dy")], "test-nullclines_ref-ex10.rds")
})
...
test_that("alternative formulation equal to reference behavior", {
  ...
  ex1.alt.out <- do.call(nullclines, c(list(deriv=ex1.alt, state.names = c("A")), ex1.pars))
